...
func scanImage(a *Actions, task manifest.DockerPush) Step {
	...
	step := Step{
		...
		With: With{
			"entrypoint": "/bin/sh",
			"args":       fmt.Sprintf(`-c "%s [ -f .trivyignore ] && echo \"Ignoring the following CVE's due to .trivyignore\" || true; [ -f .trivyignore ] && cat .trivyignore; echo || true; trivy image --timeout %dm --ignore-unfixed --severity CRITICAL --scanners vuln --exit-code %s %s || true"`, prefix, task.ScanTimeout, fmt.Sprint(exitCode), shared.CachePath(task, ":${{ env.GIT_REVISION }}")),
		},
	}
	return step
}
