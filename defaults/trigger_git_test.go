...
func TestGit(t *testing.T) {
	...
	t.Run("does nothing when URI is not set", func(t *testing.T) {
		...
		assert.Equal(t, trigger, defaultGitTrigger(trigger, Concourse, dummyGitBranchResolver, "concourse"))
	})
	...
	t.Run("private repos", func(t *testing.T) {
		...
		t.Run("no private key set", func(t *testing.T) {
			trigger := manifest.GitTrigger{}
			assert.Equal(t, defaults.RepoPrivateKey, defaultGitTrigger(trigger, defaults, dummyGitBranchResolver, "concourse").PrivateKey)
		})

		t.Run("private key set", func(t *testing.T) {
			privateKey := "sup"
			trigger := manifest.GitTrigger{PrivateKey: privateKey}
			assert.Equal(t, privateKey, defaultGitTrigger(trigger, defaults, dummyGitBranchResolver, "concourse").PrivateKey)
		})
	})
	...
	t.Run("public repos", func(t *testing.T) {
		...
		t.Run("http", func(t *testing.T) {
			...
			trigger := manifest.GitTrigger{}
			updatedTrigger := defaultGitTrigger(trigger, defaults, dummyGitBranchResolver, "concourse")
	...
	t.Run("project values", func(t *testing.T) {
		...
		assert.Equal(t, expectedTrigger, defaultGitTrigger(manifest.GitTrigger{}, defaults, dummyGitBranchResolver, "concourse"))
	})
	...
	t.Run("does not overwrite URI when set", func(t *testing.T) {
		...
		updated := defaultGitTrigger(trigger, defaults, dummyGitBranchResolver, "concourse")
	...
	t.Run("branch", func(t *testing.T) {
		...
		t.Run("Does nothing when on a random branch and branch is not set", func(t *testing.T) {
			...
			assert.Equal(t, "", defaultGitTrigger(trigger, Concourse, gitBranchResolver, "concourse").Branch)
		})
	...
	t.Run("shallow", func(t *testing.T) {
		defaults := Defaults{ShallowClone: true}
		var actual = func(trigger manifest.GitTrigger) bool {
			return defaultGitTrigger(trigger, defaults, dummyGitBranchResolver, "actions").Shallow
		}
		assert.True(t, actual(manifest.GitTrigger{ShallowDefined: false}))
		assert.False(t, actual(manifest.GitTrigger{ShallowDefined: true, Shallow: false}))
		assert.True(t, actual(manifest.GitTrigger{ShallowDefined: true, Shallow: true}))
	})
}
